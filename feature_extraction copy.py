...
class Signal:
    ...
    def __init__(self, raw, wrist_raw, num_frames, duration):
        ...
        print("after denoising, signal length: %d"%(len(self.denoised_signal)))
...
# Function to plot finger taps vs time
def plot_finger_taps_vs_time(D, duration, output_path, variability_score):
    '''
    Generate a line graph of number of finger taps vs time and display the variability score.
    Parameters:
        D: List of angular distances (denoised signal)
        duration: Total duration of the video in seconds
        output_path: Path to save the generated graph
        variability_score: Variability score to display on the graph
    '''
    # Calculate the time per frame
    num_frames = len(D)
    time_per_frame = duration / num_frames

    # Detect peaks (finger taps)
    peaks, _ = find_peaks(D, height=3)  # Adjust height threshold as needed

    # Calculate time for each peak
    time_stamps = [time_per_frame * peak for peak in peaks]

    print("Number of finger taps detected: %d" % (len(time_stamps)))

    # Create a histogram of finger taps over time
    bin_size = 1  # 1-second bins
    bins = np.arange(0, duration + bin_size, bin_size)
    tap_counts, _ = np.histogram(time_stamps, bins=bins)

    # Plot the graph as a line plot
    plt.figure(figsize=(10, 6))
    plt.plot(bins[:-1], tap_counts, marker='o', linestyle='-', color='blue', alpha=0.7)
    plt.xlabel('Time (s)')
    plt.ylabel('Number of Finger Taps')
    plt.title('Number of Finger Taps vs Time')

    # Display the variability score on the graph
    plt.text(0.95, 0.95, f'Variability Score: {variability_score:.2f}', 
             horizontalalignment='right', verticalalignment='top', 
             transform=plt.gca().transAxes, fontsize=12, color='red')

    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.savefig(os.path.join(output_path, 'finger_taps_vs_time_line.png'))
    plt.close()

# Function to calculate variability score
def calculate_variability(peaks, time_per_frame):
    '''
    Calculate the variability score based on the time intervals between peaks.
    Parameters:
        peaks: List of indices of detected peaks
        time_per_frame: Duration of each frame in seconds
    Returns:
        Variability score (e.g., standard deviation of intervals)
    '''
    if len(peaks) < 2:
        return 0  # Not enough peaks to calculate variability

    intervals = np.diff(peaks) * time_per_frame  # Time intervals between peaks
    variability_score = np.std(intervals)  # Standard deviation of intervals
...
def extract_features(filename, output_path, hand, labels=(0,"")):
    ...
    print("Output path: %s"%(full_dir_path))
    ...
    print("Creating directory %s"%(full_dir_path))
    ...
    print("Denoising and Interpolation...")
...
def withoutUI():
    ...
    video_path = "path_to_video2.mp4"  # Replace with your video path
    features   = extract_features(video_path, "data", "right", labels=(0,""))
